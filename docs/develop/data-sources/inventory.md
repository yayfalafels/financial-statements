...
| artifact_path |                                            |
| usage_intent  |                                            |
| 01            | data/monthly-closing/accounts.json         |
| 02            | data/monthly-closing/balance-accounts.json |
| 03            | data/monthly-closing/account-balances.csv  |
| 04            | data/monthly-closing/inputs.json           |
| 05            | data/monthly-closing/txns.json             |
| 06            | data/monthly-closing/txns.json.sample      |

| id            |                                                          |
| ------------- | -------------------------------------------------------- |
| artifact_path |                                                          |
| usage_intent  |                                                          |
| 01            | Account definitions with s3 IDs and statement patterns   |
| 02            | Balance account definitions with alias and month ID      |
| 03            | Historical manual account balance inputs by month        |
| 04            | Manual parameter overrides for the monthly close         |
| 05            | Monthly close transaction batch for income and transfers |
| 06            | Sample transaction batch showing IBKR and CPF patterns   |
...
| artifact_path |                         |
| usage_intent  |                         |
| 01            | reference/hb-finances/  |
| 02            | reference/hb-reconcile/ |
| 03            | reference/notion-bills/ |
...
Process references capture detailed user workflow evidence from historical planning artifacts. Use these sources to understand current-state execution detail when requirement docs are less granular.

| id           |                                   |
| ------------ | --------------------------------- |
| artifact     |                                   |
| usage_intent |                                   |
| 01           | notion monthly closing process[1] |

| id           |                                                                   |
| ------------ | ----------------------------------------------------------------- |
| artifact     |                                                                   |
| usage_intent |                                                                   |
| 01           | Detailed current-state monthly close process and source/tool flow |
